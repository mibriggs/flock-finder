...
<div class="relative flex min-w-[200px] flex-col gap-1 rounded-xl p-2">
	<button
		class="absolute top-2 right-2 flex h-6 w-6 items-center justify-center rounded-full bg-red-500 text-white transition-all hover:bg-red-600 active:scale-90 active:bg-red-700"
		onclick={onClose}
	>
		<X size={14} />
	</button>
	<h3 class="pr-8 text-lg font-bold text-slate-800">{commonName}</h3>
	<p class="text-sm text-slate-600 italic">{scientificName}</p>
